...
export const LATEST_SNAPSHOT_KEY = "data/latest-fuelprices.csv";
export const LATEST_MANIFEST_KEY = "data/latest-fuelprices.json";
...
const DEFAULT_REQUEST_HEADERS = {
  ...
  "User-Agent":
    "Mozilla/5.0 (compatible; MobileMeFuelRefresh/1.0; +https://ciaranmcaleer.is-a.dev/MobileMe/)",
};

export function getFuelFinderApiBaseUrl(environment = DEFAULT_ENVIRONMENT) {
  if (environment !== "production" && environment !== "test") {
    throw new Error(`Unsupported Fuel Finder environment: ${environment}`);
  }
...
export function getFuelFinderConfigFromEnv(environmentSource) {
  const clientId = environmentSource.FUEL_FINDER_CLIENT_ID?.trim();
  const clientSecret = environmentSource.FUEL_FINDER_CLIENT_SECRET?.trim();
  const environment = environmentSource.FUEL_FINDER_ENVIRONMENT?.trim() || DEFAULT_ENVIRONMENT;
  ...
  return {
    apiBaseUrl: getFuelFinderApiBaseUrl(environment),
    clientId,
    clientSecret,
    environment,
  };
...
export async function fetchFuelFinderAccessToken({
  fetchImpl,
  apiBaseUrl,
  clientId,
  clientSecret,
...
async function fetchPaginatedCollection({ fetchImpl, apiBaseUrl, token, endpoint }) {
  const rows = [];

  for (let batchNumber = 1; ; batchNumber += 1) {
    const targetUrl = new URL(`${apiBaseUrl}${endpoint}`);
    targetUrl.searchParams.set("batch-number", String(batchNumber));
...
export async function fetchFuelFinderSnapshot({
  fetchImpl,
  apiBaseUrl,
  clientId,
  clientSecret,
}) {
  const accessToken = await fetchFuelFinderAccessToken({
    fetchImpl,
    apiBaseUrl,
    clientId,
    clientSecret,
...
export function buildFuelSnapshotCsv({ prices, forecourts }) {
...
export function buildSnapshotStoragePlan(currentTime = new Date()) {
  ...
  return {
    ...
    latestCsvKey: LATEST_SNAPSHOT_KEY,
    latestManifestKey: LATEST_MANIFEST_KEY,
    dailyCsvKey: `data/history/daily/${isoDate}/latest-fuelprices.csv`,
    dailyManifestKey: `data/history/daily/${isoDate}/manifest.json`,
    versionedCsvKey: `data/history/runs/${year}/${month}/${safeTimestamp}/latest-fuelprices.csv`,
    versionedManifestKey: `data/history/runs/${year}/${month}/${safeTimestamp}/manifest.json`,
  };
}
...
export function buildSnapshotManifest({
  ...
  return {
    ...
    forecourtsCount,
    latest: {
      csvKey: storagePlan.latestCsvKey,
      manifestKey: storagePlan.latestManifestKey,
    },
    daily: {
      csvKey: storagePlan.dailyCsvKey,
      manifestKey: storagePlan.dailyManifestKey,
    },
    versioned: {
      csvKey: storagePlan.versionedCsvKey,
      manifestKey: storagePlan.versionedManifestKey,
    },
  };
}
